...
(function () {
    setTimeout(() => {
        //get the bot
        fetch(`${homeurl}/api/bots/referrer`, {
            method: "POST",
            body: JSON.stringify({
                bot: { id: botid },
            }),
            headers: {
                ...
                "x-bot-referrer": window.location.href,
            },
        })
            .then((response) => {
                response.json().then((bot) => {
                    console.log(bot);
                    if (bot._id == botid) {
                        putStyles(bot);
...
function putStyles(bot) {
    ...
    const themeHoverBackGoundColor = bot.themeColors.themeHoverBackGoundColor;
    const botTextWrapperColor = bot.themeColors.themeTextWrapperColor;//getLightColorFromBg(bot.talkBoxParams.talkboxBackGround);
    ...
    optionStyles.innerText = `.bot-text-wrapper { background-color: ${botTextWrapperColor}; padding: 7px;  margin-top: 12px; margin-left: 3px; width: 85%; border-radius: 5px; padding-bottom: 15px; } .text { padding: 2px; font-weight: 400; letter-spacing: 1px; font-family: inherit; } .options-container { padding: 2px;   margin-top: 5px; padding-top: 2px; } .options-container div { margin-right: 5px; letter-spacing: 1px;  box-shadow: 1px 3px 9px 0px ${themeBoxShadowColor}; color:#ffffff; font-family: Arial; background-color:${themePrimaryColor}; padding: 3px 7px; font-size:16px; font-weight:bold; text-decoration:none; border-radius: 4px; display: inline-flex; width: max-content; text-shadow:0px 1px 0px ${themeTextShadowColor}; } .options-container div:hover { background-color: ${themeHoverBackGoundColor}; cursor: pointer; } .options-container div:active { position:relative; top:1px; } .rounded { border-radius:50% !important; } .multi-container { padding: 2px; margin: 5px 5px 5px 12px; padding-top: 2px; max-height: 200px; overflow-y: scroll; } .multi-container div { margin: 7px; letter-spacing: 1px; box-shadow: 1px 3px 9px 0px ${themeBoxShadowColor}; padding: 3px 7px; text-decoration:none; user-select: none; border-radius: 4px; display: inline-flex; width: max-content; } .multi-btn { display: inline-flex; letter-spacing: 1px; line-height: 0; padding: 15px; box-shadow: 0px 10px 14px -7px ${themeBoxShadowColor}; background-color:${themePrimaryColor}; border-radius: 5px; margin: 10px; font-family:Arial; font-size:16px; font-weight:bold; text-decoration:none; text-shadow:0px 1px 0px ${themeTextShadowColor}; color:#ffffff; } .multi-btn:hover { background-color:${themeHoverBackGoundColor}; } .multi-btn:active { position:relative; top:1px; } .res-text { padding-right: 12px; margin: 7px; background-color: ${themeHoverBackGoundColor}; color: white; letter-spacing: 1px; font-family: inherit; padding-left: 12px; margin-left: 11%; border-radius: 4px;   min-width: 100px; } .btn-container-text { text-align: center; box-shadow: 0px 2px 5px 0px ${themeBoxShadowColor}; background-color: ${themePrimaryColor}; border-radius: 4px; cursor: pointer; color: #ffffff; font-family: Arial; font-size: 16px; font-weight: bold; letter-spacing: 1px; padding: 8px 1px; text-decoration: none; margin: 9px; text-shadow: 0px 1px 0px ${themeTextShadowColor};} btn-container-text:hover { background-color: ${themeHoverBackGoundColor}; } .btn-container-text:active { position:relative; top:1px; } #loader { width:fit-content; } .arrow-right { width: 0; height: 0; border-top: 12px solid transparent; border-bottom: 12px solid transparent;  border-left: 25px solid ${themePrimaryColor}; position: relative; top: -34px; float: right; } .arrow-right:hover { border-left: 25px solid ${themeHoverBackGoundColor}; } .branding { text-align: center; font-size: medium; font-family: monospace; letter-spacing: 2px; padding-top: 6px; user-select: none; transition: ease 2s } .cal-cover { top: 12% !important; }`;
...
function sendToServer(q, a) {
    ...
    let _session = botid.substr(0, 4) + btoa(new Date().toString()) + botid.substr(4, botid.length);
    if (!document.getElementById('session').value) {
        document.getElementById('session').value = _session;
    }
    else {
        _session = document.getElementById('session').value;
    }
    console.log('send to server -- >   Q:', JSON.stringify(a), ' A:', JSON.stringify(a));
    fetch(`${homeurl}/api/bots/botresponse`, {
        method: "POST",
        body: JSON.stringify({
            bot: { id: botid },
            botresponse: { botId: botid, question: q, answer: a, session: _session },
        }),
        headers: {
            Accept: "application/json",
            "Content-type": "application/json; charset=UTF-8",
