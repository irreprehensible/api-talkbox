(function () {
  ...
  var botId = str.split("/")[str.split("/").length - 1].split(".")[0];
  fetch("http://localhost:7000/api/bots/verify", {
    method: "POST",
    body: JSON.stringify({
      bot: { id: botId },
    }),
    ...
    if (!data) {
      ...
      data.json().then((result) => {
        if (result.name) {
          ...
          talkBoxIframe.srcdoc =
            '<!DOCTYPE html><head><script> var botId = "61c49d32e9588a31cc2d93be" </script> </head> <body> <div id="chat-container"> <div id="chat-box"></div> <div id="type-box"> <div class="branding"></div>  </div> </div> <input type="hidden" id="session" /> <div> <script src="http://localhost:7000/cdn/talkbox.js"></script> </div> </body> </html>';
